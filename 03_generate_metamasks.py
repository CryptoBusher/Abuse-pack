"""
Script for logging to Metamask accounts in dolphin profiles
"""
...
def generate_metamask_accounts(_dolphin_profile_ids: list, _metamask_password: str):
    """
    Function for generating to metamask accounts in bulk
    ...
    for _i, dolphin_profile_id in enumerate(_dolphin_profile_ids):
        ...
        created_metamask_account = metamask_account.create_metamask_account(_webdriver)

        if not created_metamask_account:
            print(f"Failed to create metamask account for profile {dolphin_profile_id}")
        else:
            fr.append_txt_file("data/generated_metamasks",
                               f"{metamask_account.public_key}:{metamask_account.seed_phrase}")
            print(f"Successfully created and saved to .txt metamask account for profile {dolphin_profile_id}")
    ...
    print(f"Finished creating metamask accounts")
...
if __name__ == "__main__":
    ...
    generate_metamask_accounts(dolphin_profile_ids, config["metamask_password"])
